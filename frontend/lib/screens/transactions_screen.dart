...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalReceived = (summary['total_received'] ?? 0.0).toDouble();
    final count = summary['count'] ?? 0;

    // Group transactions by date
    final Map<String, List<dynamic>> grouped = {};
    for (final t in txns) {
      final String fullDate = (t['txn_date'] ?? 'Unknown').toString();
          ...
          Expanded(
            child: finance.isLoading
                ...
                ? _searchQuery.isNotEmpty
                      ...
                      : _EmptyTransactions(themeColor: themeColor)
                : RefreshIndicator(
                    color: themeColor,
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final dayTotal = dayTxns.fold<double>(
                          0,
                          (s, t) {
                            final isRec = t['txn_type'] == 'received' || (t['notes']?.toString().contains('Received:') ?? false);
                            return s + (isRec ? 0.0 : (t['amount'] ?? 0.0));
                          },
                        );
                        return _DayGroup(
                          date: date,
  ...
  void _showAddSheet(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _AddTransactionSheet(finance: finance, existing: existing),
    );
  }
}
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${context.read<SettingsProvider>().currency}${_fmtFull(total, context.read<SettingsProvider>().conversionRate)}',
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${context.read<SettingsProvider>().currency}${_fmtFull(totalReceived, context.read<SettingsProvider>().conversionRate)}',
                      style: const TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        color: Colors.green,
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Text(
                '${context.read<SettingsProvider>().currency}${_fmt(dayTotal, context.read<SettingsProvider>().conversionRate)}',
                style: TextStyle(
                  color: themeColor,
                  fontSize: 13,
                  fontWeight: FontWeight.bold,
...
class _TxnTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = t['title']?.toString().isNotEmpty == true
        ? t['title']
        : _catName(t['category_id']);
    final notes = t['notes']?.toString() ?? '';

    // Check if received
    final isReceived = t['txn_type'] == 'received' || notes.contains('Received:');
                       
    // Extract timestamp if available
    String timeStr = '';
    final fullDate = t['txn_date']?.toString() ?? '';
    if (fullDate.contains('T') || fullDate.contains(' ')) {
      try {
    ...
    final bottomText = timeStr.isNotEmpty ? '$method • $timeStr' : method;
      ...
      confirmDismiss: (_) async {
        return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text('Remove this transaction entry?'),
            actions: [
              TextButton(
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '${isReceived ? '+' : ''}${context.read<SettingsProvider>().currency}${_fmt(amount.abs(), context.read<SettingsProvider>().conversionRate)}',
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 15,
...
class _AddTransactionSheet extends StatefulWidget {
  ...
  const _AddTransactionSheet({required this.finance, this.existing});
  @override
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Center(
              child: ToggleButtons(
                isSelected: [_txnType == 'sent', _txnType == 'received'],
                onPressed: (idx) => setState(() => _txnType = idx == 0 ? 'sent' : 'received'),
...
class _EmptyTransactions extends StatelessWidget {
  final Color themeColor;
  const _EmptyTransactions({required this.themeColor});
  @override
  Widget build(BuildContext context) {
    return Center(
      child: Column(
