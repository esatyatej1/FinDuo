...
class _PhonePeImportScreenState extends State<PhonePeImportScreen> {
  ...
  Future<void> _checkStatus() async {
    ...
    await GmailService.tryRestoreSession();
    setState(() {
  ...
  Future<void> _loadTransactions() async {
    ...
    // Auth checks
    if (_useGmail && !GmailService.isSignedIn) {
      final success = await GmailService.signIn();
      if (!success) {
        setState(() { _useGmail = false; });
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gmail sign-in failed. Proceeding without Gmail.')));
      }
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('✅ Imported $imp transactions. Skipped $skip duplicates.'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Smart Sync', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      ),
      body: _loading
                        ...
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (_, i) => _TxnImportCard(
                              ...
                              onToggle: () => setState(() => _transactions[i].selected = !_transactions[i].selected),
      ...
      bottomNavigationBar: !_loading && _error == null && _transactions.isNotEmpty
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [themeColor.withValues(alpha: 0.15), themeColor.withValues(alpha: 0.05)]),
            ...
            children: [
              Row(
                children: [
                  ...
                  const Text('Background Auto-Sync', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  const Spacer(),
                  Switch(
              ...
              const Text('Checks SMS for UPI debits every 2 hours and imports them silently.', style: TextStyle(fontSize: 13, color: Colors.grey)),
              const SizedBox(height: 12),
              Row(
                children: [
                  const Text('Last synced: ', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  Text(_lastSyncStr, style: TextStyle(fontSize: 12, color: themeColor, fontWeight: FontWeight.bold)),
                ],
              ),
            ],
          ),
        ...
        const Text('Manual Sync', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ...
        SwitchListTile(
          ...
          subtitle: const Text('Read PhonePe & Bank payment emails'),
          value: _useGmail,
          onChanged: (v) => setState(() => _useGmail = v),
            ...
            onPressed: () async {
              final picked = await showDateRangePicker(
                ...
                builder: (ctx, child) => Theme(
                  data: Theme.of(ctx).copyWith(colorScheme: ColorScheme.dark(primary: themeColor)),
                  child: child!,
                ),
              );
              if (picked != null) {
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            onPressed: (!_useSms && !_useGmail) ? null : _loadTransactions,
            icon: const Icon(Icons.search_rounded),
            label: const Text('Scan for Transactions', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
        ),
      ],
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          Text('Found $total unique transactions',
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton.icon(
          ...
          label: Text(
            _importing ? 'Importing...' : 'Import $count Transaction${count != 1 ? 's' : ''}',
  ...
  Widget _buildLoading(Color themeColor) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(height: 16),
      const Text('Correlating SMS & Gmail data...', style: TextStyle(color: Colors.grey)),
    ]),
  );
  ...
  Widget _buildError(Color themeColor) => Padding(
    ...
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.error_outline_rounded, size: 64, color: Colors.redAccent.withValues(alpha: 0.6)),
      const SizedBox(height: 16),
...
class _TxnImportCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final subs = categoryId != null ? finance.subCategoriesOf(categoryId!) : <dynamic>[];
          ...
          ListTile(
            ...
            leading: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                height: 28,
                decoration: BoxDecoration(
                  color: txn.selected ? const Color(0xFF5F259F) : Colors.transparent,
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  Text(txn.date, style: TextStyle(fontSize: 11, color: Colors.grey[500])),
                  if (txn.refNo.isNotEmpty) ...[
                    const SizedBox(width: 10),
                    Icon(Icons.tag_rounded, size: 11, color: Colors.grey[500]),
                    const SizedBox(width: 2),
                    Text(txn.refNo, style: TextStyle(fontSize: 11, color: Colors.grey[500])),
                  ],
                ]),
                ...
                Container(
                  ...
                  child: Text(txn.sourceLabel, style: const TextStyle(fontSize: 9, color: Colors.grey)),
                ),
              ],
            ),
          ),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<int?>(
                      ...
                      items: [
                        const DropdownMenuItem(value: null, child: Text('No category', style: TextStyle(fontSize: 12))),
                        ...finance.parentCategories.map((c) => DropdownMenuItem<int>(
                            value: c['id'],
